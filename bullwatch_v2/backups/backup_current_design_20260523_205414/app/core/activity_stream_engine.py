...
def _collect_from_opportunities() -> int:
    ...
    count = 0
    try:
        from app.core.opportunity_engine import get_opportunities
        opps = get_opportunities(limit=20, hours=1)
        for opp in opps:
            evt = {
...
def _collect_from_news_impact() -> int:
    ...
    try:
        ...
        impacts = get_trending_impacts(hours=2, limit=10)
        for imp in impacts:
...
def _collect_from_alerts() -> int:
    ...
    try:
        ...
        triggered = get_triggered(limit=10)
...
def _collect_from_strategy_live() -> int:
    ...
    try:
        ...
        signals = live_engine.get_live_signals(limit=10)
        for sig in signals:
            evt = {
...
def _collect_from_live_rooms() -> int:
    ...
    try:
        ...
        rooms = list_rooms(status="live", limit=5)
        for room in rooms:
            evt = {
