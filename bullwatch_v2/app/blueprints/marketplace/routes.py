...
# ══════════════════════════════════════════════════════════════════
# AKADEMI — COURSES / TRAINING (FAZ 33)
# ══════════════════════════════════════════════════════════════════

@marketplace_bp.route("/api/marketplace/courses", methods=["GET"])
def api_marketplace_courses():
    """List all published courses for the Akademi tab in Marketplace."""
    try:
        ...
        _logger.error(f"Error fetching courses: {e}")
        return jsonify({
            "ok": False,
            "error": str(e),
        }), 500
...
@marketplace_bp.route("/api/marketplace/course/<slug>", methods=["GET"])
def api_marketplace_course_detail(slug: str):
    """Get course detail with all modules."""
    try:
        ...
        modules = CourseModule.query.filter_by(course_id=course.id).order_by(CourseModule.order).all()
        ...
        _logger.error(f"Error fetching course detail: {e}")
        return jsonify({
            "ok": False,
            "error": str(e),
        }), 500
