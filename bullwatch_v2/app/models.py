"""
axiom.fi Database Models — Supabase PostgreSQL Schema

Production-grade models for:
- User authentication and profiles
- Legal consent tracking (audit trail)
- Social trading posts (immutable records)
- Analyst success statistics
- Live mentoring rooms and chat
"""
...
# ══════════════════════════════════════════════════════════════════
# COURSES — FAZ 33 — MARKETPLACE SUB-TAB
# ══════════════════════════════════════════════════════════════════

class Course(db.Model):
    """Eğitim Kursu — Marketplace'in Akademi Alt-Tabı
    
    Kurslar Marketplace'in bir alt-bileşenidir (nested view).
    Yeni sayfa oluşturmaz, DOM içinde in-place flip ile açılır.
    """
    ...
    thumbnail_url = db.Column(db.String(500), nullable=True)  # Card görüntüsü
    
    # Mentor bilgisi
    mentor_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=True, index=True)
    mentor = db.relationship("User", backref="courses", lazy="joined")
    
    # Modüller relationship (cascade on ONE side only)
    ...
    # Kurs metadata
    level = db.Column(db.String(50), default="beginner", nullable=False)  # beginner, intermediate, advanced
    market = db.Column(db.String(50), default="crypto", nullable=False)  # crypto, stocks, forex
    pricing = db.Column(db.String(50), default="free", nullable=False)  # free, premium
    price_usd = db.Column(db.Float, default=0.0, nullable=False)
    
    # Modüller (JSON listesi olarak saklanır)
    modules_count = db.Column(db.Integer, default=0, nullable=False)
    ...
    # Composite indexes
    __table_args__ = (
        Index("ix_course_published", "is_published"),
        Index("ix_course_market_level", "market", "level"),
    )
    ...
    def to_dict(self):
        ...
        return {
            ...
            "mentor_name": self.mentor.username if self.mentor else "ZKR Academy",
            "level": self.level,
            "market": self.market,
    ...
    def __repr__(self) -> str:
        return f"<Course {self.slug} level={self.level}>"


class CourseModule(db.Model):
    """Kurs Modülü — Her Kursun Bölümleri
    
    Modüller sıralı videolar, quiz, materyallerdir.
    """
    ...
    description = db.Column(db.Text, nullable=True)
    order = db.Column(db.Integer, default=0, nullable=False)  # Sıralama
    ...
    # Relationships (remove cascade from many-to-one side, define on Course instead)
    
    # Indexes
    __table_args__ = (
        Index("ix_module_course_order", "course_id", "order"),
    )
    ...
    def __repr__(self) -> str:
        return f"<CourseModule course={self.course_id} order={self.order}>"
